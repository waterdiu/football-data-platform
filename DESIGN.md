...
- `reports/world_cup_referee_profile_gap_report.json` 由 `scripts/build_world_cup_referee_profile_gap_report.py` 生成，用 52 名 FIFA 世界杯主裁和本地 `official-ratings` 做样本覆盖对账。当前只有 Michael Oliver 与 Anthony Taylor 命中本地英超历史样本，且样本数超过 50；其余 50 名主裁仍需 worldfootball.net、WorldReferee 或 API-FOOTBALL Pro 按比赛聚合补国际/洲际执法样本后，才能形成生产强画像。该报告会读取已完成的 WorldReferee probe 并输出 `worldreferee_experimental_summary` 与每名主裁的 `worldreferee_experimental_sample`：当前 47 个 WorldReferee 候选页被判定有效，36 名主裁有实验概要统计，25 名达到报告解释样本门槛，21 名达到风格蒸馏样本门槛，14 名达到强模型信号样本门槛；`reports/worldreferee_referee_strong_profile_candidates.json` 进一步输出这些实验强画像候选的 `metrics`、`confidence` 和 `style_tags`。这些数据仍为 `experimental_report_only`，不得进入 `normalized/public` 或作为模型强信号，除非完成来源条款、ID 匹配、字段稳定性和可重复性审查。
