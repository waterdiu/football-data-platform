...
- `player-profiles.json` 对当前 234 名已导入官方名单球员输出 `field_coverage`、`impact_box` 和 `pending_source` basis。当前通过 Reep `key_transfermarkt` 和已审计的 dcaribou activity Transfermarkt ID 映射接入 dcaribou Transfermarkt 离线数据，补充 `player-external-facts.json` 221 条；其中 211 条有俱乐部，221 条有 DOB/age。`shirt_number` 仍无可靠来源，保持 `null + pending_source`。这些第三方事实只补充 profile，不覆盖官方 roster master。
...
- `referee-profiles.json` 当前发布 219 条可渲染官方/裁判 profile：170 条 FIFA 世界杯比赛官员名单 + 49 条英超历史裁判样本唯一人物。历史样本源文件有一条大小写重复的 `L Mason/l Mason`，合并后按唯一 `official_id` 保留 49 条历史人物。裁判基础身份字段包括 `official_id/person_id`、姓名、国籍/协会、`country_code`、`association_code`、`confederation`、`role/role_zh`、`roles`、`assignment_status`、`assigned_matches`、`fifa_listed_since`、`source_url`、`source_refs` 和 `updated_at`。当前 52 名 FIFA 主裁已全部补齐 `confederation`，分布为 AFC 8、CAF 7、CONCACAF 9、CONMEBOL 12、OFC 1、UEFA 15。`date_of_birth` 与 `age` 已进入顶层和 `direct` 契约；当前 52 名 FIFA 主裁中 44 名通过高置信 Wikidata 身份匹配补齐 DOB/age，来源写入 `official-external-facts.json` 和 `direct.external_identity_facts[]`，其余官员保持 `null + pending_identity_source`。`direct.assigned_matches=[]`，世界杯条目 `assignment_status=pending_match_assignment`。
...
- 球员、教练、裁判 profile 顶层均保留 `date_of_birth` 和 `age` 作为消费端便捷字段；事实来源仍以 `direct.field_sources` 和 `direct.field_coverage` 为准。裁判 DOB/age 不能从 FIFA 官员 PDF、football-data.co.uk 历史样本或 WorldReferee 概要稳定获得；当前仅发布 `reports/wikidata_official_identity_probe_report.json` 中 `confidence=high` 且 DOB 为完整 ISO 日期的 Wikidata 身份事实，低置信、无 DOB 或只有年份精度的候选继续停留在报告中等待人工复核。
...
- 人物数据源就绪度由 `scripts/build_person_data_source_readiness.py` 生成到 `reports/person_data_source_readiness.json`。截至 2026-05-18，本地已可读取人工下载的 dcaribou DuckDB，包含 `games`、`appearances`、`game_events`、`game_lineups`、`club_games` 等表；因此可生成历史出场、分钟、进球助攻、牌、历史号码候选和身价历史。`scripts/build_player_external_facts.py` 也会复用 `person_player_dcaribou_activity_master.json` 中已审计的 Transfermarkt ID，当前额外补入 22 名韩国队球员 DOB/age/club。`shirt_number` 与真实 `absence_impact_pct` 仍不能由该源直接生成：前者缺少 FIFA 官方 2026 号码，后者还需要伤停/可用性 evidence 与球队表现反事实样本。`salimt/football-datasets` 仍是 probe-only：GitHub license metadata 为 null，根目录没有 `LICENSE`，`contents/LICENSE` 返回 404，递归 tree 未发现 license/copying/notice 文件。
...
- `reports/world_cup_referee_profile_gap_report.json` 由 `scripts/build_world_cup_referee_profile_gap_report.py` 生成，用 52 名 FIFA 世界杯主裁和本地 `official-ratings` 做样本覆盖对账。当前只有 Michael Oliver 与 Anthony Taylor 命中本地英超历史样本，且样本数超过 50；其余 50 名主裁仍需 worldfootball.net、WorldReferee 或 API-FOOTBALL Pro 按比赛聚合补国际/洲际执法样本后，才能形成生产强画像。该报告会读取已完成的 WorldReferee probe 并输出 `worldreferee_experimental_summary` 与每名主裁的 `worldreferee_experimental_sample`：当前 47 个 WorldReferee 候选页被判定有效，36 名主裁有实验概要统计，25 名达到报告解释样本门槛，21 名达到风格蒸馏样本门槛，14 名达到强模型信号样本门槛；`reports/worldreferee_referee_strong_profile_candidates.json` 进一步输出这些实验强画像候选的 `metrics`、`confidence`、`dimension_ratings`、`risk_modifiers` 和 `style_tags`。这些数据仍为 `experimental_report_only`，不得进入 `normalized/public` 或作为模型强信号，除非完成来源条款、ID 匹配、字段稳定性、可重复性审查和回测。
