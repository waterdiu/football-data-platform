...
- 已归档：数据源调研与缺口总览文档为 `docs/2026-05-19-data-source-research-status-cn.md`。该文档集中记录 `worldcup/2026` 与 `world-cup-predictor` 的数据需求、平台已提供数据、稳定性、赔率/高级统计/人物数据源 probe 结论，以及无法获取的原因分类。FBref live probe 显式允许浏览器驱动后仍长时间无输出，只能作为低频实验/缓存源；雷速 GitHub 项目仅确认旧页面爬虫可解析 1X2/AH/OU/比分字段，保持 `experimental_only`；Odds-API.io 仍需 `ODDS_API_IO_KEY` 后才能 live 验证。
...
- 已新增：API-FOOTBALL Pro 使用手册 `docs/2026-05-19-api-football-pro-usage-runbook-cn.md`。如果升级 $19/月 Pro（7,500 requests/day），平台按请求预算管理：正常比赛日目标 <1,000 请求，高峰日 <1,500 请求，保留至少 2,500 请求安全余量；先用 coverage 和批量 `/fixtures?ids=...` 降低请求，再按 T-window 采 lineups/injuries/odds，赛后批量归档 events/statistics/players。P0/P1 完成且 remaining 充足时，剩余请求可进入历史补强队列，用于 48 队近 20/50 场、H2H、球员赛季/伤停历史等模型迭代数据；历史补强默认只进 reports/data/model，不直接覆盖 public。
