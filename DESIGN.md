...
- 已归档：数据源调研与缺口总览文档为 `docs/2026-05-19-data-source-research-status-cn.md`。该文档集中记录 `worldcup/2026` 与 `world-cup-predictor` 的数据需求、平台已提供数据、稳定性、赔率/高级统计/人物数据源 probe 结论，以及无法获取的原因分类。FBref live probe 显式允许浏览器驱动后仍长时间无输出，只能作为低频实验/缓存源；雷速 GitHub 项目仅确认旧页面爬虫可解析 1X2/AH/OU/比分字段，本次复查未找到可靠当前维护项目，保持 `metadata_only_no_current_repo` / `experimental_only`；HKJC `hkjc-api` 为 MIT 非官方 wrapper，字段覆盖有价值，但 direct football `matches/matchResult` 查询返回 `WHITELIST_ERROR`，保持 `experimental_blocked_live`；Odds-API.io 仍需 World Cup event 可见性复查后才能进入下一步。
