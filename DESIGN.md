...
- 已新增：Odds-API.io 小型采样实验脚本为 `scripts/sample_odds_api_io_snapshots.py`。该脚本只写 `data/raw/experimental/odds-api-io` 与 `reports/odds_api_io_sampling_report.json`，用于验证 `ML`/`Spread`/`Totals` 到平台标准 1X2/AH/OU schema 的映射稳定性；不得写 `data/normalized`、`data/model` 或 public API。
