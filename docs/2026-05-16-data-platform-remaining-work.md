...
- News-derived injury evidence is supported through `scripts/build_world_cup_injury_evidence.py`. It adds `absence_evidence_summary` to injury rows from `prematch_context` injury/suspension mentions. This is evidence only, not an official availability verdict.
