...
7. Continue Sofascore experimental field-coverage probe for match statistics, lineups, player ratings, shotmap/xG, and possible PPDA inputs. If direct endpoint probes remain blocked, only evaluate specialized wrappers such as `tunjayoff/sofascore_scraper`, `pysofascore`, or `ScraperFC`; keep all live payloads under `data/raw/experimental/sofascore` and out of normalized/public until authorization review.
