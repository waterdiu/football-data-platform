...
- 赔率数据价值高，但生产风险也高。
- 2026-05-19 live probe 只确认 `https://www.leisu.com/` 首页可匿名返回比赛列表与 match id；`data.leisu.com`、`odds.leisu.com`、单场详情页、数据分析页和情报页被 WAF / UA ACL 阻断，不能作为生产采集源。
...
|---|---|
| 数据覆盖 | 高，尤其赔率/盘口/比分 |
| 稳定性 | 低；当前单场详情和赔率 live 请求被 WAF 阻断 |
| 合规风险 | 高 |
| 上手成本 | 中到高 |
| 是否进生产 | 否 |
