...
- 覆盖：232 名已映射或兜底匹配球员，其中 198 名有出场/分钟汇总，220 名有历史号码候选，213 名有事件汇总，175 名有身价历史
- 韩国队补齐规则：Reep 缺少 24 名韩国队球员的 `key_transfermarkt`。导入脚本现在使用 dcaribou DuckDB 的“国家 + 姓名倒序唯一匹配”兜底，已补 22 名。`Kim Taehyeon` 因同名候选歧义、`Lee Kihyuk` 因罗马化差异仍保持未自动匹配，需要人工证据。
- 未匹配报告：`reports/dcaribou_person_activity_unresolved_report.json`，仅用于人工审计，不进入 public API。
